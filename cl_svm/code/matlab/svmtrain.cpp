...
int read_problem_dense(const mxArray *label_vec, const mxArray *instance_mat)
{
	...
	for(i = 0; i < prob.l; i++)
	{
		#ifdef _DENSE_REP
			prob.x[i].dim = sc;
			prob.x[i].values = (double*) calloc( sizeof(double), (sc + 1) );
			prob.y[i] = labels[i];
		#else
			prob.x[i] = &x_space[j];
			prob.y[i] = labels[i];
...
int read_problem_sparse(const mxArray *label_vec, const mxArray *instance_mat)
{
	...
	j = 0;
		...
		for(i=0;i<prob.l;i++)
		{
			...
			for(k=low;k<high;k++)
			{
				if ( ( ir[k] ) > maxValueIndex )
				{
					maxValueIndex = ( ir[k] + 1 );
				}
			}
		}
	#else
	...
	for(i=0;i<prob.l;i++)
	{
		...
		#ifdef _DENSE_REP
			prob.x[i].dim = maxValueIndex;
			prob.x[i].values = (double*) calloc( sizeof(double), maxValueIndex );
			prob.y[i] = labels[i];
		#else
			prob.x[i] = &x_space[j];
			prob.y[i] = labels[i];
		...
		low = (int)jc[i], high = (int)jc[i+1];
			...
			for ( k = low; k < high; k++ )
			{
				prob.x[ i ].values[ (int)ir[k] ] = samples[k];
			}
		#else
			for(k=low;k<high;k++)
			{
		...
		int nrhs, const mxArray *prhs[] )
{
	...
	if(nrhs > 1 && nrhs < 4)
	{
		...
		mexPrintf( "Finished reading problem\n" );
		...
		{
			mexPrintf( "About to do actual training\n" );
			int nr_feat = (int)mxGetN(prhs[1]);
			const char *error_msg;
			model = svm_train(&prob, &param);
			mexPrintf( "Finished actual training\n" );
			error_msg = model_to_matlab_structure(plhs, nr_feat, model);
			mexPrintf( "Converted model to matlab structure\n" );
			if(error_msg)
