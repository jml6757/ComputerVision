	...
	// SVs
	{
		...
		else
		{
			...
			for(i = 0; i < model->l; i++) {
				j = 0;
				#ifdef _DENSE_REP
					nonzero_element = nonzero_element + model->SV[i].dim;
				#else
		...
		for(i = 0;i < model->l; i++)
		{
			...
			else
			{
				int x_index = 0;
				#ifdef _DENSE_REP
					for ( x_index = 0; x_index < model->SV[i].dim; x_index++, ir_index++ )
					{
						// ?
						ir[ ir_index ] = x_index;
...
struct svm_model *matlab_matrix_to_model(const mxArray *matlab_struct, const char **msg)
{
	...
	// SV
	{
		...
		// SV is in column
		#ifdef _DENSE_REP
			// Need to get the max index
			maxIndex = 0;
			low = (int)jc[i], high = (int)jc[i+1];
			for(j=low;j<high;j++)
			{
				if ( ( (int)ir[j] + 1 ) > maxIndex )
				{
					maxIndex = (int)ir[j] + 1;
				}
			}
		...
		for(i=0;i<sr;i++)
		{
			...
			#ifdef _DENSE_REP
				// TODO
				model->SV[i].dim = maxIndex;
				model->SV[i].values = (double*) calloc( sizeof(double), maxIndex );
