...
//	do_cross_validation();
	...
	else
	{
		...
		// debugging
		fprintf( stdout, "Finished training model\n" );
		...
		// debugging
		fprintf( stdout, "Finished saving model\n" );
		svm_free_and_destroy_model(&model);
		// debugging
		fprintf( stdout, "Freed and destroyed model\n" );
	}
	svm_destroy_param(&param);
	
	// debugging
	fprintf( stdout, "Destroyed param\n" );
	
	free(prob.y);

	// debugging
	fprintf( stdout, "Freed prob.y\n" );
	...
	// debugging
	fprintf( stdout, "Freed all the other stuff\n" );
...
void read_problem(const char *filename)
{
	...
	for(i=0;i<prob.l;i++)
	{
		...
		{
			// debugging
			fprintf( stderr, "LABEL IS AT END OF LINE\n" );
		...
		while(1)
		{
			...
			{
				// debugging
				fprintf( stderr, "ERROR WHILE READING IN VECTOR INDEX: %s, %i\n", idx, ((prob.x+i)->dim) );
				fprintf( stderr, "%s\n", line );
				exit_input_error(i+1);
			}
			else
			...
			{
				// debugging
				fprintf( stderr, "ERROR WHILE READING IN VECTOR VALUE: %i\n", j );
				exit_input_error(i+1);
			}
